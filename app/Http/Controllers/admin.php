...
class admin extends Controller
{
    ...
    public function show_dashboard()
    {
        ...
        $teacher = DB::table('tbl_teachers')
            ...
            ->where(['soft_delete' => '0'])
            ->get();
        $pelajaran = DB::table('tbl_pelajarans')
            ->select('kode_pelajaran', 'nama_pelajaran')
            ->where(['soft_delete' => '0'])
            ->get();
    ...
    public function do_login(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (Auth::guard('admin')->attempt($login, $request->remember)) {
                DB::table('tbl_logs')
                    ->insert([
                        'no_pegawai'=>Auth::guard('admin')->user()->no_pegawai,
                    ]);
        ...
        switch ($request->user_type) {
            case 'student':
                ...
                if ($validator->fails()) {
                    ...
                    $checkup = DB::table('tbl_students')
                        ->select('soft_delete')
                        ->where(['nisn' => $request->username,'soft_delete'=>'1'])
                        ->first();

                    if (!empty($checkup) && $checkup->soft_delete == '1') {
                        DB::table('tbl_students')
                            ->where(['nisn' => $request->username])
                            ->update([
                                'soft_delete' => '0'
                            ]);
    ...
    public function do_create_pelajaran(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $checkup = DB::table('tbl_pelajarans')
                ->select(['soft_delete'])
